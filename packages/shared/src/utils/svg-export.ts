...
function getMatrix(data: string, ecl: string): boolean[][] | null {
  try {
    const qr = QRCode.create(data, { errorCorrectionLevel: ecl as "L" | "M" | "Q" | "H" });
...
export function generateSVG(
  ...
  if (qrStyle.logoUri && !skipLogo) {
    ...
    const lr = qrStyle.logoStyle?.background === "circle" ? logoArea / 2
      : qrStyle.logoStyle?.background === "rounded" ? logoArea * 0.2
      : 0;
    if (qrStyle.logoStyle?.background && qrStyle.logoStyle.background !== "none" && qrStyle.logoStyle.background !== "square") {
      svgParts.push(`<clipPath id="logoclip"><rect x="${lx}" y="${ly}" width="${logoArea}" height="${logoArea}" rx="${lr}"/></clipPath>`);
      svgParts.push(`<image href="${qrStyle.logoUri}" x="${lx}" y="${ly}" width="${logoArea}" height="${logoArea}" clip-path="url(#logoclip)" preserveAspectRatio="xMidYMid slice"/>`);
      if (qrStyle.logoStyle?.border) {
        svgParts.push(`<rect x="${lx}" y="${ly}" width="${logoArea}" height="${logoArea}" rx="${lr}" fill="none" stroke="${qrStyle.bgColor}" stroke-width="${logoPad * 0.6}"/>`);
      }
    } else {
      svgParts.push(`<image href="${qrStyle.logoUri}" x="${lx}" y="${ly}" width="${logoArea}" height="${logoArea}" preserveAspectRatio="xMidYMid slice"/>`);
    }
  }

  svgParts.push(`</svg>`);

  return svgParts.filter(Boolean).join("\n");
}
